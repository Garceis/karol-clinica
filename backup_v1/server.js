...
/* ================= ROTAS DOCUMENTOS ================= */
app.post('/api/documentos/upload', upload.single('arquivo'), (req, res) => {
...
app.get('/api/documentos/:paciente_id', (req, res) => {
...
/* ================= ROTAS LINKS TESTES ================= */
app.post('/api/testes/gerar-link', (req, res) => {
...
app.put('/api/pacientes/:id', (req, res) => {
    const { id } = req.params;
    const { nome, cpf, nome_mae, idade, telefone, observacao, status } = req.body;
    try {
        db.prepare('UPDATE pacientes SET nome = ?, cpf = ?, nome_mae = ?, idade = ?, telefone = ?, observacao = ?, status = ? WHERE id = ?')
            .run(nome, cpf, nome_mae, idade, telefone, observacao, status, id);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
app.patch('/api/pacientes/:id/status', (req, res) => {
    ...
    try {
        db.prepare('UPDATE pacientes SET status = ? WHERE id = ?').run(status, id);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
/* ================= ROTAS ANAMNESE ================= */
app.get('/api/anamneses/:paciente_id', (req, res) => {
    const { paciente_id } = req.params;
    try {
...
app.post('/api/anamneses', (req, res) => {

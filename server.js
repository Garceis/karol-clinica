...
app.post('/api/auth/login', (req, res) => {
    ...
    const identificador = usuario || email; // suporte aos dois campos
    try {
        const user = db.prepare('SELECT * FROM admin_users WHERE email = ? OR email = ?').get(identificador, identificador);
...
app.post('/api/testes/submeter', (req, res) => {
    ...
    const transaction = db.transaction(() => {
        const srs2Formatado = Object.entries(dados_json)
            .filter(([key]) => key !== 'respondente_nome')
            .map(([key, val]) => {
                const questoes = {
                    '1': 'Não é bem verdade', '2': 'Às vezes é verdade', '3': 'Frequentemente é verdade', '4': 'Quase sempre é verdade'
                };
                const num = key.replace('q', '');
                return `Questão ${num}: ${val} - ${questoes[val] || val}`;
            }).join('\n');
            
        const historicoFinal = `Respondente: ${dados_json.respondente_nome || 'Não informado'}\n\nResultados SRS-2:\n${srs2Formatado}`;

        db.prepare('INSERT INTO anamneses (paciente_id, queixa_principal, historico, data) VALUES (?, ?, ?, ?)')
          .run(paciente_id, 'Avaliação Externa SRS-2', historicoFinal, new Date().toISOString().split('T')[0]);
...
app.post('/api/pacientes', verificarToken, (req, res) => {
    const { nome, cpf, nome_mae, idade, telefone, observacao, status, anamnese_completa, data_nascimento, valor_sessao } = req.body;
    const transaction = db.transaction(() => {
        const info = db.prepare('INSERT INTO pacientes (nome, cpf, nome_mae, idade, telefone, observacao, status, data_nascimento, valor_sessao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
            .run(nome, cpf, nome_mae, idade, telefone, observacao, status || 'ativo', data_nascimento, valor_sessao);
...
app.put('/api/pacientes/:id', verificarToken, (req, res) => {
    ...
    const transaction = db.transaction(() => {
        db.prepare('UPDATE pacientes SET nome = ?, cpf = ?, nome_mae = ?, idade = ?, telefone = ?, observacao = ?, status = ?, data_nascimento = ?, valor_sessao = ? WHERE id = ?')
            .run(nome, cpf, nome_mae, idade, telefone, observacao, status || 'ativo', data_nascimento, valor_sessao, id);
...
app.post('/api/anamneses', verificarToken, (req, res) => {
    ...
    const transaction = db.transaction(() => {
        ...
        // Automação Financeira: Se houver valor_sessao definido, lançar no financeiro
        const p = db.prepare('SELECT valor_sessao, nome FROM pacientes WHERE id = ?').get(paciente_id);
        if (p && p.valor_sessao > 0) {
            db.prepare('INSERT INTO financeiro (paciente_id, descricao, valor, tipo, data) VALUES (?, ?, ?, ?, ?)')
              .run(paciente_id, `Sessão: ${p.nome}`, p.valor_sessao, 'entrada', data);
        }
...
/* ================= ROTAS DECLARAÇÕES ================= */
app.get('/api/declaracoes/:paciente_id', verificarToken, (req, res) => {
    const { paciente_id } = req.params;
    try {
        const rows = db.prepare('SELECT * FROM declaracoes WHERE paciente_id = ? ORDER BY data DESC').all(paciente_id);
        res.json(rows);
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
app.post('/api/declaracoes', verificarToken, (req, res) => {
    const { paciente_id, tipo, conteudo, data } = req.body;
    try {
        const info = db.prepare('INSERT INTO declaracoes (paciente_id, tipo, conteudo, data) VALUES (?, ?, ?, ?)')
            .run(paciente_id, tipo, conteudo, data);
        res.json({ id: info.lastInsertRowid, success: true });
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
/* ================= ROTAS DASHBOARD / LEMBRETES ================= */
app.get('/api/dashboard/aniversariantes', verificarToken, (req, res) => {
    try {
        ...
        const rows = db.prepare(`SELECT id, nome, data_nascimento FROM pacientes WHERE status = 'ativo' AND strftime('%m', data_nascimento) = ? AND strftime('%d', data_nascimento) = ? ORDER BY nome ASC`).all(month, day);
        res.json(rows);
    } catch (err) {
        res.status(500).json({ success: false, message: err.message });
    }
...
// Site institucional (raiz do projeto)
...
// Admin React (painel)
app.use('/painel', express.static(frontendPath));
...
app.use((req, res, next) => {
    ...
    // Se for rota do admin, servir o index do React
    if (req.path.startsWith('/painel') || req.path.startsWith('/dashboard') || req.path.startsWith('/responder')) {
        return res.sendFile(path.join(frontendPath, 'index.html'));
    }
    // Caso contrário, tentar servir o React (para rotas como /#/dashboard via hash)

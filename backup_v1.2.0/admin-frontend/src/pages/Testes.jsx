...
export default function Testes() {
  ...
  const fetchPacientes = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3000/api/pacientes?status=ativo', {
  ...
  const gerarLink = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('http://localhost:3000/api/testes/gerar-link', {
      ...
      const fullUrl = `http://localhost:3000/avaliacao.html?token=${response.data.token}`;

      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-8">
            ...
            <div className="space-y-4" onDragOver={onDragOver} onDrop={onDrop}>
              ...
              <label className="cursor-pointer group block">
                <div className="border-2 border-dashed border-slate-200 rounded-[2rem] p-6 text-center hover:bg-teal-50 hover:border-teal-300 transition-all duration-300">
                  <div className="h-10 w-10 bg-slate-100 rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
                    <Upload className="h-5 w-5 text-slate-500 group-hover:text-teal-600" />
                  </div>
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest leading-tight">Arraste arquivos ou<br/><span className="text-teal-600">clique aqui</span></p>
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white p-2 rounded-[2rem] shadow-sm border border-slate-100 flex gap-2">
            {[
              { id: 'historico', label: 'Evolução Clínica', icon: History, activeColor: 'bg-slate-800' },
              { id: 'anamnese', label: 'Anamnese Completa', icon: BookOpen, activeColor: 'bg-teal-500' },
              { id: 'nova', label: 'Registrar Sessão', icon: Plus, activeColor: 'bg-sky-500' }
            ].map(tab => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex-1 py-4 px-6 rounded-2xl text-xs font-black uppercase tracking-[0.1em] flex items-center justify-center gap-3 transition-all duration-300 ${activeTab === tab.id ? `${tab.activeColor} text-white shadow-xl scale-[1.02]` : 'text-slate-400 hover:bg-slate-50'}`}
              >
                        ...
                        {Object.entries(secVal).map(([field, val]) => (
                          val && field !== 'texto' && field !== 'obs_finais' && (
                            <div key={field} className="p-4 bg-slate-50/70 rounded-2xl border border-slate-200/50 hover:bg-white hover:shadow-md transition-all">
                              ...
                              <span className="text-xs text-slate-700 font-bold whitespace-normal break-words">
          ...
          {activeTab === 'nova' && (
            <div className="bg-white p-8 sm:p-12 rounded-[3.5rem] shadow-sm border border-slate-100">
              <form onSubmit={handleSalvarSessao} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Data da Sessão</label>
                    <input type="date" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-sky-500 outline-none font-bold text-slate-700" value={novaSessao.data} onChange={e => setNovaSessao({...novaSessao, data: e.target.value})} required />
                  </div>
                  <div className="md:col-span-2 space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Queixa Principal / Assunto</label>
                    <input type="text" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-sky-500 outline-none font-bold text-slate-700" value={novaSessao.queixa_principal} onChange={e => setNovaSessao({...novaSessao, queixa_principal: e.target.value})} placeholder="Ex: Regulação emocional e marcos motores" required />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Relatório de Evolução</label>
                  <textarea className="w-full p-6 bg-slate-50 border border-slate-200 rounded-[2rem] focus:ring-2 focus:ring-sky-500 outline-none h-80 text-slate-700 leading-relaxed font-medium" value={novaSessao.historico} onChange={e => setNovaSessao({...novaSessao, historico: e.target.value})} placeholder="Registre aqui o desenvolvimento da sessão, observações comportamentais e orientações..." required />
                </div>
                <button type="submit" className="w-full py-5 bg-sky-500 text-white rounded-3xl shadow-xl shadow-sky-500/20 hover:bg-sky-600 transition-all font-black text-xl active:scale-95 flex items-center justify-center gap-3">
                  <Save className="h-6 w-6" /> Finalizar Registro
                </button>
              </form>
            </div>
          )}
